...
+ alfalfa, whole state only: <https://quickstats.nass.usda.gov/results/1DA15606-E8E3-35D7-A971-E5590F5AEC35>
...
```{r}
alfalfa.by.state <- read_csv("../2-Data/Raw/Yields/IA-alfalfa-state-61B0BB3A-2F85-3644-B6F5-19805AFF9766.csv")
```
...
IA.yield.NASS <- rbind(corn.soy.by.county, oat.by.county,
                       alfalfa.by.county, alfalfa.by.state) %>%
  dplyr::select(Year, County, Commodity, `Data Item`, Value) %>% 
  mutate(Commodity = tolower(Commodity)) %>%
  mutate(`Value metric` = ifelse(Commodity == "corn", Value * 56 * 0.000454 / 0.404686,
                                ifelse(Commodity == "soybeans", Value * 60 * 0.000454 / 0.404686,
                                       ifelse(Commodity == "oats", Value * 32 * 0.0004534 / 0.404686,
                                              Value * 0.907185 / 0.404686 )))) %>%
  rename(`Value imperial` = Value) %>%
  mutate(`Metric unit` = ifelse(Commodity == "alfalfa", "t/ha", "Mg/ha"))
```
...
#  rename(`% moisture` =  `% moisture...7`) %>%
  ...
  mutate(`lb/ac at 14% moisture` = `bu/acre at 14% moisture` * 32,
    ...
    select(Plot, Treatment, Year,
          `bu/acre at 14% moisture`,`ton/ac at 14% moisture`, `Mg/ha at 14% moisture`, 
         `ton/ac at 0% moisture`, `Mg/ha at 0% moisture`) %>%
  rename(`bu/acre at 14%`= `bu/acre at 14% moisture`)
```
