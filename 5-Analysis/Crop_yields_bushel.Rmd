...
#### Corn
```{r, include=FALSE}
corn.lmer <- lmer(`bu/acre at 15.5% moisture` ~ Block +
...
corn.lmer.ln <- lmer(log(`bu/acre at 15.5% moisture`) ~ Block +
                    Crop_ID*Corn_weed_management + 
                    (1|Year) + 
                    (1|Year:Block) + 
                    (1|Year:Crop_ID) +
...
corn.lmer.s <- lmer(sqrt(`bu/acre at 15.5% moisture`) ~ Block +
                    Crop_ID*Corn_weed_management + 
                    (1|Year) + 
                    (1|Year:Block) + 
                    (1|Year:Crop_ID) +
...
#### Corn diagnosis: `corn.lmer` best

```{r, fig.height= 4, fig.width=12, fig.cap= "Model diagnosis for Marsden corn yield. Transformation doesn't improve model fitting"}
...
```{r}
soy.lmer <- lmer(`bu/acre at 13% moisture` ~ Block +  Crop_ID*Corn_weed_management + 
                    (1|Year) + 
                    (1|Year:Block) + 
                    (1|Year:Crop_ID) +
                    (1|Year:Corn_weed_management) + 
...
soy.lmer.ln <- lmer(log(`bu/acre at 13% moisture`) ~ Block +  Crop_ID*Corn_weed_management + 
                    (1|Year) + 
                    (1|Year:Block) + 
                    (1|Year:Crop_ID) +
                    (1|Year:Corn_weed_management) + 
...
soy.lmer.s <- lmer(sqrt(`bu/acre at 13% moisture`) ~ Block +  Crop_ID*Corn_weed_management + 
                    (1|Year) + 
                    (1|Year:Block) + 
                    (1|Year:Crop_ID) +
                    (1|Year:Corn_weed_management) + 
...
#### Soybean diagnosis: `soy.lmer` best

```{r, fig.height= 4, fig.width=12, fig.cap= "Model diagnosis for Marsden soybean yield. Transformation doesn't improve model fitting"}
...
```{r}
oat.lmer <- lmer(`bu/ac at 14% moisture` ~ Block + 
                   Crop_ID + 
                   (1|Year) +
                   (1|Year:Block) +  
                   (1|Year:Crop_ID), 
...
oat.lmer.ln <- lmer(log(`bu/ac at 14% moisture`) ~ Block + 
                   Crop_ID + 
                   (1|Year) +
                   (1|Year:Block) +  
                   (1|Year:Crop_ID), 
...
oat.lmer.s <- lmer(sqrt(`bu/ac at 14% moisture`) ~ Block + 
                   Crop_ID + 
                   (1|Year) +
                   (1|Year:Block) +  
                   (1|Year:Crop_ID), 
...
#### Oat diagnosis: `oat.lmer.ln` best

```{r, fig.height= 4, fig.width=12, fig.cap= "Model diagnosis for Marsden oat yield. Transformation doesn't model fitting"}
...
alf_sum <- alfalfa %>%
  group_by(Year, Block) %>%
  summarize(Yield.ton.ac = sum(`ton/ac at 0% moisture`)) 
...
#### Alfalfa diagnosis: `alf.lmer` best

```{r, fig.height= 4, fig.width=12, fig.cap= "Model diagnosis for Marsden oat yield. Transformation doesn't improve model fitting"}
...
crop_jt_ct <- cbind(crop_df, crop_ct_df[,-c(3:5)])
...
colnames(crop_jt_ct_df) <- c("Source of variation", "df1", "df2","F","p", "contrast", "difference", "p")
...
IA_avg_yield <- IA_yield %>% 
  group_by(Commodity) %>%
  summarise(IA_avg_Bu = mean(Value),
            IA_sd_Bu = sd(Value))

Boone_yield <- IA_yield %>%
  filter(County == "Boone") %>%
  group_by(Commodity) %>%
  summarise(Boone_avg_Bu = mean(Value),
            Boone_sd_Bu = sd(Value))
...
ggplot(aes(x = Crop_ID, y = yvar, fill = Rotation))+ 
  ...
  geom_hline(aes(yintercept = IA_avg_Bu, linetype = "IA average")) + 
  geom_hline(aes(yintercept = Boone_avg_Bu, linetype = "Boone County average")) + 
  ylab("Crop yield (Bu/ac)") +
  xlab("Crop identity") +
  labs(linetype = "Yield") +
