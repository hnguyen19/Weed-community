...
```{r, include=FALSE}
corn.lmer <- lmer(log(Standardized_yield_bu_ac) ~ Block +
...
soy.lmer <- lmer(log(Standardized_yield_bu_ac) ~ Block +  Crop_ID*Corn_weed_management + 
                    (1|Year) + 
                    (1|Year:Block) + 
                    (1|Year:Crop_ID) +
                    (1|Year:Corn_weed_management) + 
...
oat.lmer <- lmer(log(Standardized_yield_bu_ac) ~ Block + 
                   Crop_ID + 
                   (1|Year) +
                   (1|Year:Block) +  
                   (1|Year:Crop_ID) + 
...
alf_sum <- alfalfa %>%
  group_by(Year, Block) %>%
  summarize(Yield = sum(Dry_ton_p_ac)) 
